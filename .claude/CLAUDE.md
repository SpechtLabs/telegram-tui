...
- **`tracing-batteries`** is pinned but unused: its battery installs its own global subscriber and filters only by level, which can't coexist with the file log or the allowlist. `otel.rs` drives the OTLP stack directly.
