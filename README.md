...
Built on TDLib and [ratatui](https://ratatui.rs). The binary is called `tgt`. The only modifiers in the whole application are `ctrl+p` for the command palette and `ctrl+c` to quit; everything else is arrows, Enter, Escape, and single letters that stay visible on screen while they apply. Mouse works too, if you want it.

**[▶ Watch it running](https://tgt.specht-labs.de/#what-using-it-looks-like).** A short recording: the chat list, a reply, a reaction, a spoiler getting revealed. Made with `tgt --demo` against mock data, not a real account. GitHub can't play it inline, so it lives on the docs site.
...
Windows exists because the code turned out to be portable, not because anyone has used the client there. Its loader has no equivalent of the relocatable `bin/` + `lib/` layout the other two rely on, so there is nothing to publish. If you try it anyway, please open an issue about what broke.

Linux needs **glibc 2.39 or newer**. TDLib publishes only a prebuilt library, and this project links against it on every install route, including from source, rather than compiling TDLib itself. That prebuilt binary carries its own glibc requirement, and no build configuration on our end can lower it. Ubuntu 24.04 and later, Ubuntu 24.10, and Debian 13 (trixie) all clear it. Debian 12 bookworm (the current Debian stable), Ubuntu 22.04 LTS, and RHEL/Rocky 9 don't. Below 2.39 the binary refuses to start at all (`version 'GLIBC_2.39' not found`), whether you built from source or downloaded a release tarball, since both pull the same download. The install script checks this and tells you before it downloads anything.

The credential store needs a running Secret Service provider too, such as gnome-keyring.
...
Updates go through `brew upgrade tgt` from then on. `tgt update` refuses on a Homebrew install and tells you so: brew already tracks the files in its own manifest, and an in-place overwrite would desynchronise it.
...
On Linux, see the [glibc 2.39 floor](#platforms) above. It applies here too: building from source downloads the same prebuilt TDLib the release tarball ships.
...
A photo you have downloaded renders as the picture itself on terminals that speak kitty, iTerm2, or sixel (kitty, Ghostty, iTerm2, and WezTerm are detected automatically; sixel is opt-in with `TGT_SIXEL=1`). Everywhere else, it stays a single descriptive line, including inside tmux, which drops the escape sequences unless it's configured for passthrough. If you've set tmux's `allow-passthrough` up, `TGT_FORCE_GRAPHICS=1` re-enables detection there.

## Privacy

Crash reports and OTLP export can both leave your machine. They carry different guarantees, and a first-run screen describes each before anything is sent, before you even log in.
